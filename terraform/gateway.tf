...
resource "aws_apigatewayv2_domain_name" "domain_name" {
  domain_name = var.env == "live" ? "blurhash.${var.root_domain}" : "blurhash-${var.env}.${var.root_domain}"
...
resource "aws_apigatewayv2_route" "lambda_route_blurhash" {
  api_id           = aws_apigatewayv2_api.lambda.id
  target           = "integrations/${aws_apigatewayv2_integration.lambda.id}"
  route_key        = "GET /api/blurhash"
  api_key_required = false
  operation_name   = "blurhash"
}
##############################################################################
